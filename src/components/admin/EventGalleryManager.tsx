...
import { CloudUpload, X, Loader2, Trash2 } from 'lucide-react';
import { createClient } from '@/lib/supabase/client';
import { addEventPhoto, deleteEventPhoto } from '@/lib/mutations/eventPhotos';
...
export function EventGalleryManager({ eventId, photos }: EventGalleryManagerProps) {
  ...
  async function handleFiles(files: FileList) {
    ...
    for (const file of Array.from(files)) {
      ...
      const result = await addEventPhoto(eventId, publicUrl);
      if (!result.success) {
  ...
  return (
    <div className="space-y-4">
      {/* Uploader */}
      <div
        className="relative aspect-video bg-paper-warm border-[2.5px] border-dashed border-ink rounded-md cursor-pointer hover:bg-paper-warm/70 transition-colors"
        ...
        <div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {photos.map((photo) => (
              <div
                key={photo.id}
                className="relative aspect-square bg-paper-warm border-[1.5px] border-ink rounded-md overflow-hidden group"
              >
                <Image
                  src={photo.image_url}
                  alt=""
      ...
      {photos.length === 0 && !uploading && (
        <p className="text-xs text-ink-muted text-center py-2">
          Sin fotos cargadas aún. Subí las primeras para crear la galería pública.
        </p>
      )}
    </div>
  );
